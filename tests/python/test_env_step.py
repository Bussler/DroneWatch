...
def test_env_step_returns_rllib_multi_agent_payload() -> None:
    ...
    assert "local_reward_terms" in infos["agent_0"]
    assert "team_reward" in infos["agent_0"]
    assert "per_agent_reward" in infos["agent_0"]
    assert "episode_shared_reward" in infos["agent_0"]
    assert "episode_local_reward" in infos["agent_0"]
    assert "episode_reward_terms" in infos["agent_0"]
    ...
    assert "shared_reward_terms" not in infos["agent_0"]
    assert "shared_reward" not in infos["agent_0"]
    assert "local_reward" not in infos["agent_0"]
    assert "episode_reward" not in infos["agent_0"]
    assert "episode_shared_reward_terms" not in infos["agent_0"]
    assert "episode_local_reward_terms" not in infos["agent_0"]
    assert "episode_length" not in infos["agent_0"]
