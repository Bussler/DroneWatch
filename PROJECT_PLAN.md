...
## 24. Recommended First Task for a Local Agent

Start with the project skeleton and PyO3/maturin integration.

First concrete deliverable:

```bash
make install
python -c "import dronewatch; print('DroneWatch import OK')"
python -c "import swarm_sim; print(swarm_sim.version())"
make test
```

Then implement the Rust simulation incrementally.

Do **not** start with RLlib training before the simulation can run a random 200-step rollout from Python.
